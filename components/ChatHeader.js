...
const ChatHeader = memo(({ chat, onBack, onMoreOptions, theme, onContactPress }) => {
  ...
  return (
    <div className="app-header chat-header bg-white dark:bg-dark-secondary border-b border-gray-200 dark:border-gray-700 px-4 pt-4 flex items-center transition-colors duration-300" style={{ paddingTop: `calc(1rem + ${typeof window !== 'undefined' && window.CSS && window.CSS.env ? 'env(safe-area-inset-top, 0)' : '0px'})` }}>
      <button 
        onClick={onBack}
